          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  <Translate contentKey="eduApp.fragment.goal">Goal</Translate>
                </th>
                <th>
            ...
            <tbody>
              {fragmentList.map((fragment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {fragment.goals
                      ? fragment.goals.map((val, j) => (
                          <span key={j}>
                            <Link to={`/goal/${val.id}`}>{val.title}</Link>
                            {j === fragment.goals.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
                  <td>{fragment.targets ? <Link to={`/fragment/${fragment.targets.id}`}>{fragment.targets.id}</Link> : ''}</td>
