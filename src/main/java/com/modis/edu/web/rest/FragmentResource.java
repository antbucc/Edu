...
public class FragmentResource {
    ...
    public List<Fragment> getAllFragments(
        ...
        if ("order-is-null".equals(filter)) {
            log.debug("REST request to get all Fragments where order is null");
            return StreamSupport
                .stream(fragmentRepository.findAll().spliterator(), false)
                .filter(fragment -> fragment.getOrder() == null)
                .collect(Collectors.toList());
        }

        if ("sequence-is-null".equals(filter)) {
            log.debug("REST request to get all Fragments where sequence is null");
            return StreamSupport
                .stream(fragmentRepository.findAll().spliterator(), false)
                .filter(fragment -> fragment.getSequence() == null)
                .collect(Collectors.toList());
        }
